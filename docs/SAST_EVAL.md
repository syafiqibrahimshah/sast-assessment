...
| **Scan duration** | `--config auto`<br>Total time: 40.3134s Config time: 3.5687s Core time: 36.7354s<br><br>`-config p/security-audit` Total time: 4.0853s Config time: 0.5193s Core time: 3.5623s<br><br>`-config semgrep/rules.yml`<br>Total time: 0.8699s Config time: 0.1057s Core time: 0.7609s | `CodeQL scanned 11 out of 11 Python files and 1 out of 1 GitHub Actions files in this invocation. Typically CodeQL is configured to analyze a single CodeQL language per invocation, so check other invocations to determine overall coverage information.`<br>`real 1.99`<br>`user 5.82`<br>`sys 0.28` | **Verified against target-app:** Semgrep durations came from `--time`; CodeQL duration uses /usr/bin/time |
