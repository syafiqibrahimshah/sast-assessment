...
| **Findings returned** | 34 with `auto`; 12 with `p/security-audit` | 8 Python, 5 JavaScript, 2 Go, and 0 Java security results | **Verified against target-app:** counts came from the JSON/SARIF artifacts in `artifacts/` |
...
| **Scan duration** | See Duration measurement | See Duration measurement | **Verified against target-app:** Initially Semgrep durations came from `--time`; CodeQL duration uses /usr/bin/time but comparing with different 2 tools was biased. Used AI agent to perform analysis that uses /usr/bin/time to surpress some manual typing pain |
...
| **Rule customisability** | YAML patterns rules | QL query packs and custom queries | **Vendor documentation:** both support custom detection logic.<br><br>**Verified against target-app:** `semgrep/rules.yml` validated and ran 5 rules; standard CodeQL packs ran, with no custom QL query authored. |
